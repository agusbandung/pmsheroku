...
module.exports = function(db) {
  ...
  router.get('/details/:id/overview', userChecker, function(req, res) {
    let sqlQuery = `SELECT m.id, u.firstname || ' ' || u.lastname AS membername,
    p.name AS projectname FROM members m JOIN users u ON m.userid = u.userid
    JOIN projects p ON m.projectid = p.projectid
    WHERE m.projectid = ${req.params.id};`
  ...
  router.get('/details/:id/members', userChecker, function(req, res) {
    let filterQuery = [];
    let isFilter = false;
    let sqlQuery = `SELECT members.id, users.userid, users.firstname || ' ' || users.lastname AS name, users.position FROM members
    ...
    if(req.query.cposition && req.query.position) {
      filterQuery.push(`users.position = '${req.query.position}'`)
      isFilter = true;
    }
  ...
  router.get('/details/:id/addmember', userChecker, function(req, res) {
    db.query("SELECT * FROM users", function(err, userData) {
      ...
      db.query(`SELECT p.projectid, p.name, m.userid FROM projects p JOIN members m ON p.projectid = m.projectid WHERE p.projectid= ${req.params.id}`, function(err, data) {
        if(err) {
          console.error(err);
        }
  ...
  router.get('/details/:id/issues', userChecker, function(req, res) {
    ...
    db.query(sqlQuery, function(err, membersData) {
      ...
      db.query(sqlQuery, function(err, countData) {
        //pagination
        console.log("req.url", req.url, req.url.indexOf("page="));
        let url = (req.url.indexOf("page=")== -1) ? req.url + "/?page=1" : req.url;
        console.log("this is url (issues) variable: ", url);
        let page = Number(req.query.page) || 1
        let limit = 2
        let offset = (page-1) * 2
        let total = countData.rows[0].total;
        ...
        db.query(sqlQuery, function(err, issuesData) {
          console.log('ini issuesData : ', issuesData.rows);
  ...
  router.get('/details/:id/issues/delete/:issuesid', userChecker, function(req, res) {
    ...
    db.query(sqlQuery, function(err, issueData) {
      ...
      db.query(sqlQuery, function(err) {
        ...
        db.query(`DELETE FROM issues WHERE issuesid = ${req.params.issuesid}`, function(err) {
  ...
  router.get('/details/:id/issues/edit/:issuesid', userChecker, function(req, res) {
    let sqlQuery = `SELECT * FROM issues WHERE issuesid = ${req.params.issuesid}`
  ...
  router.post('/details/:id/issues/edit/:issuesid', userChecker, function(req, res) {
    ...
    duedate = '${dueDate}', estimatedtime = ${estimatedTime} WHERE issuesid = ${issuesid}`
    ...
    db.query(sqlQuery, function(err) {
      ...
      VALUES('${activityTitle}', '${activityDescription}', '${activityAuthor}', NOW(), '${activityCurrentDate}', '${activityHour}', ${req.params.id})`
      console.log("====================================");
      console.log("/details/:id/issues/edit/:issuesid");
  ...
  router.get('/details/:id/issues/edit/:issuesid/deleteimage/:imagename', userChecker, function(req, res) {
    let sqlQuery = `SELECT * FROM issues WHERE issuesid = ${req.params.issuesid}`;
    ...
    db.query(sqlQuery, function(err, issueData) {
      ...
      console.log("====================================");
      console.log("/details/:id/issues/edit/:issuesid/deleteimage/:imagename");
      ...
      sqlQuery = `UPDATE issues SET files = '${insertedDataFile}' WHERE issuesid = ${req.params.issuesid}`
      db.query(sqlQuery, function(err) {
        if(err) {
          console.error(err);
        }
        ...
        db.query(sqlQuery, function(err) {
          ...
          res.redirect(`/projects/details/${req.params.id}/issues/edit/${req.params.issuesid}`);
  ...
  router.post('/details/:id/issues/upload/:issuesid', userChecker, function(req, res) {
    ...
    uploadFile.mv(`${__dirname}/../public/assets/${fileName}.${fileExtension}`, function(err) {
      ...
      db.query(sqlQuery, function(err, issueData) {
        ...
        console.log("====================================");
        console.log("/details/:id/issues/upload/:issuesid");
        console.log("issueData.rows: ");
        console.log(issueData.rows);
        console.log("====================================");
  ...
  router.get('/details/:id/issues/new', userChecker, function(req, res) {
    let sqlQuery = `SELECT p.projectid, u.userid, u.firstname || ' ' || u.lastname AS membername,
    p.name AS projectname FROM members m JOIN users u ON m.userid = u.userid
    JOIN projects p ON m.projectid = p.projectid
    WHERE m.projectid = ${req.params.id};`
    db.query(sqlQuery, function(err, membersData) {
  ...
  router.post('/details/:id/issues/new', userChecker, function(req, res) {
    ...
    let sqlQuery = `INSERT INTO issues(projectid, tracker, subject, description, status,
      priority, assignee, startdate, duedate, estimatedtime, percentagedone, files, author)
      VALUES(${projectid}, '${tracker}', '${subject}', '${description}', '${status}',
      '${priority}', ${asignee}, '${startDate}', '${dueDate}', ${estimatedTime},
      ${percentageDone}, '${files}', ${asignee})`
      ...
      db.query(sqlQuery, function(err) {
        ...
        //      let activityAuthor = `${req.session.user.firstname} ${req.session.user.lastname}`
        let activityAuthor = req.session.user.userid;
        //console.log(activityAuthor);
        let activityCurrentDate = `${moment().format("YYYY-MM-DD")}`
        //let activietyAWeekAgo = `${moment().subtract(7, 'days').format('DD/MM/YYYY')}`
        let activityHour = `${moment().format("HH:mm")}`
        sqlQuery = `INSERT INTO activity(title, description, author, time, tanggal, hours, projectid)
        VALUES('${activityTitle}', '${activityDescription}', '${activityAuthor}', NOW(), '${activityCurrentDate}', '${activityHour}', ${req.params.id})`
    ...
    router.get('/details/:id/activity', userChecker, function(req, res) {
      ...
      let sqlQuery = `SELECT * FROM activity WHERE projectid = ${req.params.id} AND tanggal BETWEEN '${activietyAWeekAgo}' AND '${activityCurrentDate}';`;
      db.query(sqlQuery, function(err, isdata) {
        let activityData = isdata.rows;
        let dateViewData = [ [,],  [,], [,], [,], [,], [,], [,] ];
        for(let x = 0; x< 7; x++) {
          ...
          dateViewData[x][1] = moment(dateViewData[x][0], 'YYYY-MM-DD').format('dddd, MMMM D, YYYY')
            ...
            return moment(item.tanggal).format('YYYY-MM-DD') === dateViewData[x][0]
          }));
        }
